...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan  5 15:43:13 2021

@author: naivete5656@github.com
update by Junde Xu
...
class CMP_TRACKER:
    ...
    def getPeaks_getIndicatedPoints(self, acm, mag):
        ...
        # )
        # center = center[1:, ::-1]
        center = peak_local_max(mag, threshold_abs=self.MAG_TH, min_distance=5)
        result = []
        
        for center_cell in center.astype("int"):
            # vec : [i_shift, j_shift, z_value]
            # center_cell : [i_index, j_index]
            vec = acm[center_cell[0], center_cell[1]] 
            mag_value = mag[center_cell[0], center_cell[1]]
            #### norm without z_value
            ####
    ...
    def TRACK(self):
        ...
        pre_pos = self.getPeaks_getIndicatedPoints(pre_cmp, pre_mag)
        ...
        pre_pos = track_full[track_full[:, 0] == 1] # find last frame's track which frame id is 1
        ...
        for frame in range(2, len(self.IMAGE_NAMES)):
            ...
            mag = np.sqrt(np.sum(np.square(acm), axis=-1))
            pos = self.getPeaks_getIndicatedPoints(acm, mag)
            ...
            #     ass_log,
            # )
            pre_mag = mag.copy() # prepare cell detection for next frame
            ...
            pre_pos = track_full[track_full[:, 0] == frame].copy()# prepare track for next frame
    ...
    def saveCTK(self, log, not_ass_log, div_log, ins_log):
        ...
        log = log[np.argsort(log[:, 1])]
        track = np.zeros((len(log), 5))
        s = 0
        e = 0 
        ids = np.unique(log[:, 1])
        for i in np.unique(log[:, 1]):
            idx = np.where(log[:, 1] == i)[0]
            e = e + len(idx)
            track[s:e, :] = log[idx[np.argsort(log[idx, 0])]][:, [0, 1, 2, 3, -1]]
            s = e
        ...
        np.savetxt(os.path.join(save_dir, "track.txt"), track, fmt="%d")
        np.savetxt(os.path.join(save_dir, "new_log.txt"), new_log, fmt="%d")
        np.savetxt(os.path.join(save_dir, "log.txt"), log, fmt="%d")
        np.savetxt(os.path.join(save_dir, "end_trajectory.txt"), not_ass_log, fmt="%d")
        np.savetxt(os.path.join(save_dir, "mitosis_event.txt"), div_log, fmt="%d")
        np.savetxt(os.path.join(save_dir, "insert_position.txt"), ins_log, fmt="%d")
...
if __name__ == "__main__":   
    ...
    demo = CMP_TRACKER(
        image_names = sorted(glob.glob(path.join(images_url, '*.tif')))[0:100:itv],
...
# TODO:
    # modify searchPeak from using iteration to random read
